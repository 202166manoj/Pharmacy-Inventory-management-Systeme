...
if ($conn) {
    if (isset($_GET["action"]) && $_GET["action"] == "delete") {
        ...
        $deleteInvoicesQuery = "DELETE FROM invoices WHERE CUSTOMER_ID = $id";
        $invoiceResult = mysqli_query($conn, $deleteInvoicesQuery);

        // Check if the deletion of invoices was successful
        if ($invoiceResult) {
            // Now delete the customer
            $deleteCustomerQuery = "DELETE FROM customers WHERE ID = $id";
            $customerResult = mysqli_query($conn, $deleteCustomerQuery);
            if ($customerResult) {
                showCustomers(0);
            } else {
            ...
            echo "Error deleting invoices: " . mysqli_error($conn);
        }
    }
